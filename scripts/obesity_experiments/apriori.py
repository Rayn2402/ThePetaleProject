...
Description: This file is used to identify meaningful association
             rules between categorical values and total body fat (%) categories.
...
if __name__ == '__main__':
    ...
    df, target, cont_cols, cat_cols = get_obesity_data(manager, genomics=True)

    # Extraction of masks
    masks = extract_masks(Paths.OBESITY_MASK, k=10, l=0)

    # We run apriori experiment
    run_apriori_experiment(experiment_name='obesity',
                           df=df,
                           target=target,
                           cat_cols=cat_cols,
                           masks=masks,
