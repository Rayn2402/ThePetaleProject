...
if __name__ == '__main__':
    ...
    for gene in genes_choices:
        df, target, cont_cols, cat_cols = get_warmup_data(manager, genes=gene)
        data_dict[gene] = (df, cont_cols, cat_cols)

    # Extraction of masks
    mask_file = "l0_warmup_genes.json"
    masks = extract_masks(join(Paths.MASKS, mask_file), k=args.nb_outer_splits, l=args.nb_inner_splits)
    gnn_masks = extract_masks(join(Paths.MASKS, mask_file), k=args.nb_outer_splits, l=min(args.nb_inner_splits, 2))
    ...
    for gene in genes_choices:
        ...
        HAN experiment
        """
        if args.han and gene is not None:
