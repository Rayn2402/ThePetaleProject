...
EVALUATION_NAME = "Med_restriction_NN"

# We set the path to the recordings folder
RECORDING_PATH = join("..", "..")

# We gather the hyperparameter to be optimized
with open(join("..", "..", "Hyperparameters", "med_restriction_hyperparameters.json"), "r") as read_file:
...
evaluator = NNEvaluator(model_generator=generator, sampler=warmup_sampler, k=10, l=10, hyper_params=HYPER_PARAMS,
                        optimization_metric=RegressionMetrics.mean_absolute_error,
                        evaluation_metrics=evaluation_metrics, n_trials=1000, seed=2019,
                        evaluation_name=EVALUATION_NAME, early_stopping_activated=True, max_epochs=200,
