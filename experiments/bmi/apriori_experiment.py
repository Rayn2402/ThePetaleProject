...
if __name__ == '__main__':
    ...
    f1, f2 = join(Paths.EXPERIMENTS_RECORDS, "L1_apriori"), join(Paths.EXPERIMENTS_RECORDS, "L1_apriori_BMI")
    for f in [f1, f2]:
        if not exists(f):
            mkdir(f)
    ...
    for i in range(len(masks.keys())):
        ...
        # We clean results to only keep association rules of type (... -> BMI)
        ...
        settings = {'min_support': args.min_support, 'min_confidence': args.min_confidence,
                    'min_lift': args.min_lift, 'max_length': args.max_length, 'nb_BMI_groups': args.nb_groups}

        # We print and save all the rules
        print_and_save_apriori_rules(association_results, settings, f1, f'L1_apriori_{i}', start)

        # We print and save the rules only related to BM1
        temp_list = []
        ...
        print_and_save_apriori_rules(association_results, settings, f2, f'L1_apriori_BMI_{i}', start, save_genes=True)

    # We compute summary of apriori results for rules associated to BMI
