...
if __name__ == '__main__':
    ...
    eot_bmi_df = data_manager.get_table(EOT_BMI_TABLE)
    ...
    # We proceed to table concatenation (baselines + ages + eot bmi)
    intermediate_df2 = pd.merge(intermediate_df, eot_bmi_df, on=[PARTICIPANT], how=INNER)
    removed = [p for p in list(intermediate_df[PARTICIPANT].values) if
               p not in list(intermediate_df2[PARTICIPANT].values)]
    ...
    # We proceed to table concatenation (baselines + ages + eot bmi + body fat)
    intermediate_df3 = pd.merge(intermediate_df2, body_fat_df, on=[PARTICIPANT], how=INNER)
    intermediate_df3.drop([AGE], axis=1, inplace=True)
    ...
    # We proceed to table concatenation (baselines + ages + eot bmi + body fat + metho cortico)
    intermediate_df4 = pd.merge(intermediate_df3, metho_cortico_df, on=[PARTICIPANT], how=INNER)
    removed = [p for p in list(intermediate_df3[PARTICIPANT].values) if p not in list(intermediate_df4[PARTICIPANT].values)]
    ...
    removed = [p for p in list(intermediate_df4[PARTICIPANT].values) if p not in list(complete_df[PARTICIPANT].values)]
